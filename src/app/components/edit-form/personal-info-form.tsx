	...
	return (
		<form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
			<div>
				...
				<Input id="name" {...register("personalInfo.name", { required: true })} />
				{errors?.personalInfo?.name && <p className="text-red-500 text-sm">Name is required</p>}
			</div>

			<div>
				<Label htmlFor="email">Email</Label>
				<Input id="email" type="email" {...register("personalInfo.email", { required: true })} />
				{errors?.personalInfo?.email && <p className="text-red-500 text-sm">Email is required</p>}
			</div>
			<div>
				<Label htmlFor="phoneNumber">Phone Number</Label>
				<Input id="phoneNumber" {...register("personalInfo.phoneNumber", { required: true })} />
			</div>
			<div>
